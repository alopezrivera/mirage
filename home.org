...
(defvar custom/previous-window nil
  "Previous window.")

(defun custom/record-previous-window (orig-fun &rest args)
  (setq custom/previous-window (get-buffer-window (current-buffer)))
  (apply orig-fun args))
...
(defvar custom/window-commands '(ace-window
				       next-window
				       other-window
				       mouse-set-point)
  "Window switch commands.")

(dolist (command custom/window-commands)
  (advice-add command :around #'custom/record-previous-window))
...
(defun custom/goto-window-previous ()
  (interactive)
  (select-window custom/previous-window))
