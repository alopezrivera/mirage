...
(defun custom/org-diary-scope (orig-fun)
  "Restrict scope of Org Diary bindings to Org Diar entries.

Command scoping is applied as it is very common to exit an Org
Diary buffer /without/ closing it by calling `custom/org-diary'
(`C-d'), for example by changing to another buffer, killing the
buffer, killing the window, etc., thereby causing `custom/org-diary-mode'
to remain active, and so its bindings to be available outside
of `org-diary' buffers"
