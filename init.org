...
**** current-line-header-or-list

#+begin_src emacs-lisp

(defun custom/current-line-header-or-list ()
  (custom/current-line-regex "[[:blank:]]*[-+1-9.)*]+.*$"))
...
empty list item."
  ...
  (if (custom/current-line-empty-bullet)
    ...
    (if (custom/current-line-header-or-list)
