...
(defun custom/org-insert-heading-after-subtree ()
  (let ((prev              (point-marker))
	      (on-folded-heading (custom/org-relative-line-heading-folded)))
    (if on-folded-heading
	      (custom/org-show-minimum))
    (custom/org-insert-heading-respect-content 'org-insert-heading-respect-content)
    (if on-folded-heading
	      (save-excursion (goto-char prev) (custom/org-hide-previous-subtree)))))

#+end_src

***** TODO org-insert-subheading-after-subtree
