...
****** org-heading-margin-insert-prior
...
3. Fold it back"
  (let ((prev   (custom/get-point 'beginning-of-visual-line))
	      (folded (custom/org-relative-line-heading-folded))
	      (margin (custom/regex-match-count "\n" (custom/org-heading-margin))))
    ;; Unfold if necessary
    (if folded (org-show-subtree))
    ;; Insert heading
    (cond ((not (org-current-level)) (insert "* "))
    ...
    (if folded (save-excursion (goto-char prev) (outline-hide-subtree)))
    ;; Recover margin with following heading
    (if (> margin 0) (save-excursion (insert (make-string margin ?\n))))))
